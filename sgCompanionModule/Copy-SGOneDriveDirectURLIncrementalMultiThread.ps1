function Copy-SGOneDirectURLIncrementalMultiThread {
    [CmdletBinding()]
    param (
    ...
    foreach ($User in $UserSiteData) {
        ...
        $taskName = $currentCount + ". " + "Inc " + $User.$TargetUserPrincipalNameHeader
        ...
        switch ($PSBoundParameters.Keys -contains 'ValidationLimit') {
            $true {
                Write-Information "Validation Limit Set. No Copy initiated" -InformationAction Continue
            }
            $false {
                $sourceUserEmail = $User.$SourceUserPrincipalNameHeader
                $sourceTenantSite = $sourceTenant.site
                $sourceoneDriveUrl = $user.sourceURL
                $targetUserEmail = $User.$targetUserPrincipalNameHeader
                $targetTenantSite = $targetTenant.site
                $targetoneDriveUrl = $user.targetURL
                $jobParams = @{
                    scriptblock = [scriptblock]::Create(
                        '# Setup Source User Site
                        $null = Add-SiteCollectionAdministrator -CentralAdmin $using:sourceTenantSite -SiteUrl $using:sourceoneDriveUrl
                        $sourceOneDriveSite = Connect-Site -Url $using:sourceoneDriveUrl -UseCredentialsFrom $using:sourceTenant
                        $sourceOneDriveList = Get-List -Site $sourceOneDriveSite -Name "Documents"
                        # Setup Target User Site
                        $null = Add-SiteCollectionAdministrator -CentralAdmin $using:targetTenantSite -SiteUrl $using:targetoneDriveUrl
                        $targetOneDriveSite = Connect-Site -Url $using:targetoneDriveUrl -UseCredentialsFrom $using:targetTenant
                        $targetOneDriveList = Get-List -Site $targetOneDriveSite -Name "Documents"
                        $copysettings = New-CopySettings -OnContentItemExists IncrementalUpdate
                        $results = Copy-Content -SourceList $sourceOneDriveList -DestinationList $targetOneDriveList -TaskName $using:taskName -CopySettings $copysettings
                        ...
                        $null = Remove-SiteCollectionAdministrator -CentralAdmin $using:sourceTenantSite -SiteUrl $using:sourceoneDriveUrl 
                        $null = Remove-SiteCollectionAdministrator -CentralAdmin $using:targetTenantSite -SiteUrl $using:targetoneDriveUrl'
                    )
                    Name        = $taskName
                }
