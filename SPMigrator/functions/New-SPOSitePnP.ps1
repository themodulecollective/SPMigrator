function New-SPOSitePnP
{
    ...
    param(
        ...
        ,
        [string]$migSite #full URL to migration site (e.g. https://tenant.sharepoint.com/sites/migrationsite)
        ,
        [string]$migJobsList #name of migration jobs list (e.g. Migration Jobs)
        ,
        [string]$AdminCenterURL #full URL to SharePoint Admin Center (e.g. https://tenant-admin.sharepoint.com/)
        ,
        [string]$DestSiteURLBase #base url for destination sites (e.g. https://tenant.sharepoint.com/teams/), include trailing slash
        ,
        [string[]]$SiteOwner #site owner(s) for new sites
    ...
    foreach ($row in $URLS)
    {
        ...
        if ($null -eq $row['DestinationUrl'])
        {
            $DestSiteURL = $DestSiteURLBase + $SiteTitle
        ...
        try
        {
            ...
            if ($null -eq $Site)
            {
                ...
                {
                    New-PnPSite -Type $Template -Title $SiteTitle -Alias $SiteTitle -TimeZone $Timezone -Connection $adminconn
                    Write-Host "Site Collection $($DestSiteURL) Created Successfully!" -ForegroundColor Green
                }
                else
                {
                    New-PnPSite -Type $Template -Title $SiteTitle -Url $DestSiteURL -TimeZone $Timezone -Owner $SiteOwner -Connection $adminconn
                    Write-Host "Site Collection $($DestSiteURL) Created Successfully!" -ForegroundColor Green
                }
            ...
            #add migration team and service accounts to destination sites
            Set-PnPTenantSite -Identity $DestSiteURL -Owners $SiteOwner -Connection $adminconn
            ...
            #update item in migration jobs list
