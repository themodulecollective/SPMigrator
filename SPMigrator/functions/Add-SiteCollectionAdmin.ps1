function Add-SPMSiteCollectionAdmin {
    [cmdletbinding()]
    param(
        [parameter(Mandatory)]
    ...
    {throw('you must connect to a tenant using Connect-SPOService before running Add-SiteCollectionAdmin')}
            ...
            else
            {
                $SPMConfiguration.SourceSiteCollectionAdmins.foreach({
                    ...
                    $SiteURL.foreach({
                        $Site = $_
                        try {
                            Set-SPOUser -LoginName $LoginName -Site $Site -IsSiteCollectionAdmin $true
                        }
                        catch {
